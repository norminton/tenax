...
def analyze_at_job_locations() -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    seen_paths: set[str] = set()

    for base in AT_JOB_PATHS:
...
def collect_at_job_locations(hash_files: bool = False) -> list[dict[str, Any]]:
...
def _analyze_symlink(path: Path) -> dict[str, Any] | None:
    ...
    if _path_startswith_any(target_str, TEMP_PATH_PATTERNS):
        _record_hit(
            hits,
            reason="At job symlink target points into a temporary execution path",
            score=95,
    ...
    if USER_PATH_REGEX.search(target_str):
        _record_hit(
            hits,
            reason="At job symlink target points into a user-controlled path",
            score=90,
            preview=f"symlink -> {target_str}",
    ...
    if HIDDEN_PATH_REGEX.search(target_str):
        _record_hit(
            hits,
            reason="At job symlink target references a hidden path",
            score=75,
            preview=f"symlink -> {target_str}",
    ...
    if stat_info and stat_info.st_uid != 0:
        ...
        _record_hit(
            hits,
            reason="At job symlink is owned by a non-root account",
            score=75,
...
def _analyze_file(path: Path) -> dict[str, Any] | None:
    ...
    if stat_info:
        ...
        if stat_info.st_uid != 0:
            _record_hit(
                hits,
                reason="At job file is owned by a non-root account",
                score=70,
                preview=f"owner={owner_name}",
                category="ownership",
            )
        ...
        if mode & 0o002:
            _record_hit(
                hits,
                reason="At job file is world-writable",
                score=100,
            ...
            _record_hit(
                hits,
                reason="At job file is group-writable",
                score=60,
                preview=f"mode={oct(mode)}",
                category="permissions",
            )
    ...
    if b"\x00" in raw[:4096]:
        _record_hit(
            hits,
            reason="At job contains binary content instead of expected text job data",
            score=85,
...
def _detect_download_behavior(
    ...
    if has_download_tool and has_url:
        _record_hit(
            hits,
            reason="At job downloads content from a remote URL",
            score=55,
    ...
    if has_download_tool and any(token in line_lower for token in ("-o ", "--output", "> /", ">> /")):
        if _contains_high_risk_path(line_lower):
            _record_hit(
                hits,
                reason="At job downloads remote content into a high-risk local path",
                score=70,
                preview=_with_line_number(line_number, line),
...
def _detect_pipe_to_interpreter(
    ...
    if PIPE_TO_INTERPRETER_REGEX.search(line):
        _record_hit(
            hits,
            reason="At job pipes downloader output directly into an interpreter",
            score=100,
            preview=_with_line_number(line_number, line),
...
def _detect_interpreter_one_liners(
    ...
    if INTERPRETER_ONE_LINER_REGEX.search(line):
        ...
        if any(term in line_lower for term in high_signal_terms):
            _record_hit(
                hits,
                reason="At job contains a high-risk interpreter one-liner",
                score=70,
                preview=_with_line_number(line_number, line),
                category="one-liner",
            )
...
def _detect_reverse_shells(
    ...
    for regex in SOCKET_IMPLANT_REGEXES:
        if regex.search(line):
            _record_hit(
                hits,
                reason="At job contains reverse-shell or socket-based execution behavior",
                score=100,
                preview=_with_line_number(line_number, line),
...
def _detect_encoded_execution(
    ...
    if ENCODED_TO_EXEC_REGEX.search(line):
        _record_hit(
            hits,
            reason="At job decodes content and immediately executes it",
            score=95,
            preview=_with_line_number(line_number, line),
    ...
    for regex in ENCODED_EXEC_REGEXES:
        if regex.search(line):
            _record_hit(
                hits,
                reason="At job contains encoded payload handling logic",
                score=45,
                preview=_with_line_number(line_number, line),
                category="encoded",
            )
...
def _detect_temp_or_user_exec(
    ...
    if _contains_high_risk_path(line_lower):
        if DIRECT_EXEC_REGEX.search(line):
            if _path_startswith_any(line_lower, TEMP_PATH_PATTERNS):
                _record_hit(
                    hits,
                    reason="At job executes content from a temporary path",
                    score=90,
                    preview=_with_line_number(line_number, line),
                    category="temp-exec",
                )
                ...
                _record_hit(
                    hits,
                    reason="At job executes content from a user-controlled path",
                    score=85,
                    preview=_with_line_number(line_number, line),
                    category="user-exec",
                )
    ...
    if HIDDEN_PATH_REGEX.search(line):
        _record_hit(
            hits,
            reason="At job references a hidden executable or payload path",
            score=75,
            preview=_with_line_number(line_number, line),
...
def _detect_ld_hijack(
    ...
    if _path_startswith_any(variable_value, TEMP_PATH_PATTERNS):
        _record_hit(
            hits,
            reason=f"At job sets {variable_name} to a temporary path",
            score=95,
            preview=_with_line_number(line_number, line),
            category="ld-hijack",
        )
        ...
        _record_hit(
            hits,
            reason=f"At job sets {variable_name} to a user-controlled path",
            score=90,
            preview=_with_line_number(line_number, line),
            category="ld-hijack",
        )
...
def _detect_path_hijack(
    ...
    for part in path_parts:
        if _path_startswith_any(part, TEMP_PATH_PATTERNS):
            _record_hit(
                hits,
                reason="At job modifies PATH to include a temporary directory",
                score=85,
                preview=_with_line_number(line_number, line),
        ...
        if USER_PATH_REGEX.search(part):
            _record_hit(
                hits,
                reason="At job modifies PATH to include a user-controlled directory",
                score=80,
                preview=_with_line_number(line_number, line),
                category="path-hijack",
            )
...
def _detect_stealth_or_privilege_changes(
    ...
    for regex in STEALTH_PERSISTENCE_REGEXES:
        if regex.search(line):
            ...
            _record_hit(
                hits,
                reason="At job contains stealth or privilege-manipulation logic",
                score=85,
                preview=_with_line_number(line_number, line),
...
def _detect_process_detach(
    ...
    for regex in PROCESS_DETACH_REGEXES:
        if regex.search(line):
            _record_hit(
                hits,
                reason="At job uses detached process execution behavior",
                score=25,
                preview=_with_line_number(line_number, line),
...
def _detect_suspicious_direct_exec(
    ...
    if _contains_high_risk_path(line_lower):
        if _path_startswith_any(line_lower, TEMP_PATH_PATTERNS):
            _record_hit(
                hits,
                reason="At job directly executes content from a temporary path",
                score=90,
                preview=_with_line_number(line_number, line),
                category="temp-exec",
            )
        ...
        if USER_PATH_REGEX.search(line):
            _record_hit(
                hits,
                reason="At job directly executes content from a user-controlled path",
                score=85,
                preview=_with_line_number(line_number, line),
                category="user-exec",
            )
    ...
    for matched_path in path_matches:
        if SUSPICIOUS_FILE_EXT_REGEX.search(matched_path):
            if _path_startswith_any(matched_path, TEMP_PATH_PATTERNS):
                _record_hit(
                    hits,
                    reason="At job executes a script or binary from a temporary path",
                    score=90,
                    preview=_with_line_number(line_number, line),
                    category="temp-exec",
                )
            ...
            if USER_PATH_REGEX.search(matched_path):
                _record_hit(
                    hits,
                    reason="At job executes a script or binary from a user-controlled path",
                    score=85,
                    preview=_with_line_number(line_number, line),
                    category="user-exec",
                )
...
def _apply_compound_behavior_bonuses(hits: dict[str, dict[str, Any]]) -> None:
    ...
    if any("download" in category for category in categories) and any(
        ...
        _record_hit(
            hits,
            reason="At job combines download behavior with active execution logic",
            score=35,
            preview=None,
    ...
    if "encoded" in categories and any(
        ...
        _record_hit(
            hits,
            reason="At job combines encoded payload handling with execution behavior",
            score=25,
            preview=None,
    ...
    if "path-hijack" in categories and any(
        ...
        _record_hit(
            hits,
            reason="At job combines PATH hijacking with additional suspicious execution behavior",
            score=25,
            preview=None,
...
def _is_metadata_line(line: str) -> bool:
    stripped = line.strip()
    return any(stripped.startswith(prefix) for prefix in AT_METADATA_PREFIXES)
